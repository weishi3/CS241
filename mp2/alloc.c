...
typedef struct _DATAdata { 
 	size_t size; 
 	char used; 
 	struct _DATAdata* next; 
	struct _DATAdata* prev; 

 
	char mem[]; 
} DATAdata; 

#define PTR(p) (&((DATAdata *)(p))[-1]) 
#define DATASIZE sizeof(DATAdata) 

DATAdata* nowstart = NULL;
...
void *malloc(size_t size)
{
	...
	while(derp != NULL) { 
 	...
 	DATAdata* herp = sbrk(size + DATASIZE); 
...
void *realloc(void *ptr, size_t size)
{
	...
	DATAdata* xyzzy = PTR(ptr);
	if(((DATAdata*)xyzzy)->size >= size) { 
 	...
 	if(((DATAdata*)xyzzy)->size < size) { 
 		...
 		size_t bytes = ((DATAdata*)xyzzy)->size; 
